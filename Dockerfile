...
RUN		apt-get -y install 	gawk wget git diffstat unzip texinfo gcc-multilib \
					...
					libncurses5 libncurses5-dev libncursesw5-dev libelf-dev zlib1g-dev bc sudo
																
RUN		apt-get -y install locales
RUN 		dpkg-reconfigure -f noninteractive locales
RUN 		echo 'en_US.UTF-8 UTF-8' > /etc/locale.gen && locale-gen en_US.UTF-8
RUN 		update-locale LC_ALL=en_US.UTF-8 LANG=en_US.UTF-8
RUN 		adduser --disabled-password --gecos '' ni-smartracks-dev
RUN 		adduser ni-smartracks-dev sudo
